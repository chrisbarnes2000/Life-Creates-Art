# LifeCreatesArt

Fine art portfolio, gallery management, and creative showcase platform for Tina Barnes.
Webmaster & Lead Developer: **Chris Barnes** (Founder of [RapportVerse](https://rapprt.space)).

## Documentation
The core project documentation is maintained in the **[/Docs/](./Docs/)** directory. 

- **[Resume Excerpt (1–3 Min Read)](./Docs/RESUME_SUMMARY.md)**
- **[Full Project Summary](./Docs/PROJECT_SUMMARY.md)**
- **[Quick Start Guide](./Docs/README.md)**
- **[Architecture & Structure](./Docs/STRUCTURE.md)**
- **[Project Roadmap](./Docs/INDEX_ROADMAP.md)**

## Development
To start the local development server:
```bash
npm run dev
```

